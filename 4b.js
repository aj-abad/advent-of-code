...
const checkWin = () => {
  ...
  boards.forEach((board, i) => {
    board.forEach(row => {
      if (row.every(cell => cell === null) && winningBoards.indexOf(i) < 0) {
        winningBoards.push(i)
      }
    })
    for (let j = 0; j < board.length; ++j) {
      const col = board.map(row => row[j])
      if (col.every(cell => cell === null) && winningBoards.indexOf(i) < 0)
        winningBoards.push(i)
    }
  })
}
...
//Nested loops ftw; we want to exit early when a board wins so no forEach this time
let winningNum = null
for (let h = 0; h < numbers.length; ++h) {
  const currentNumber = numbers[h]
  ...
  for (let i = 0; i < boards.length; ++i) {
    ...
    for (let j = 0; j < board.length; ++j) {
      ...
      for (let k = 0; k < row.length; ++k) {
        const cell = row[k]
        if (cell === currentNumber) row[k] = null
        checkWin()
        winningNum = currentNumber
      }
    }
  }
}

console.log(boards[winningBoards[winningBoards.length - 1]].map(el => el.reduce((a, b) => a + b)).reduce((a, b) => a + b) * winningNum)
