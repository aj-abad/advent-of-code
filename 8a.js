...
const processedInput = input.map(el => {
  const inputOutput = el.split(' | ')
  return {
...
processedInput.forEach((x) => {
  x.input.forEach((y, j) => {
    x.input[j] = y.split("").sort().join("")
  })
  x.output.forEach((y, j) => {
    x.output[j] = y.split("").sort().join("")
  })
})

//basically sudoku
processedInput.forEach(x => {
  const digits = x.input
  x.legend = new Map()
  const map = x.legend


  const digit1 = (digits.find(el => el.length === 2))
  const digit4 = digits.find(el => el.length === 4)
  const digit7 = (digits.find(el => el.length === 3))
  const digit8 = digits.find(el => el.length === 7)
  const a = digit7.split("").filter(el => digit1.indexOf(el) === -1)
  map.set("a", a.join(""))
  let digit0 = null
  const bd = digit4.split("").filter(el => digit1.indexOf(el) === -1)
  bd.forEach(el => {
    const zero = digits.filter(digit => digit.length === 6
    ).filter(x => !x.includes(el)).join()
    if (zero) {
      const d = digit4.split("").filter(x => zero.indexOf(x) === -1)
      digit0 = zero
      map.set("d", d.join(""))
    }
    else {
  ...
  const digit5 = digits.filter(el => el.length === 5 && el.includes(map.get("b"))).join("")

  const fc = digit1.split("")
  fc.forEach(el => {
    map.set(digit5.includes(el) ? "f" : "c", el)
  })

  const digit9 = digits.filter(el => el.length === 6 && el.includes(map.get("c")) && el.includes(map.get("d"))).join("")
  const e = digit0.split("").filter(el => digit9.split("").indexOf(el) < 0).join("")
  map.set("e", e)

  const g = digit8.split("").filter(el => (digit4 + map.get("a") + map.get("e")).split("").indexOf(el) < 0).join("")
  map.set("g", g)


  const digit6 = digits.filter(el => el.length === 6 && !el.includes(map.get("c"))).join("")
  const digit2 = digits.filter(el => el.length === 5 && !el.includes(map.get("f"))).join("")
  const digit3 = digits.filter(el => el.length === 5 && !el.includes(map.get("e")) && !el.includes(map.get("b"))).join("")
...
let count = 0;
processedInput.forEach(el => {
  const map = el.legend
  const digits = el.output
  const digitsToCount = [map.get(1), map.get(4), map.get(7), map.get(8)]
  digits.forEach(el => {
    if (digitsToCount.indexOf(el) >= 0) {
      count++
    }
  })
})
